...
function(process_and_install_metainfo)
	...
	# Keep every generated/staged file in the build tree. Packaging builds mount
	# the source read-only so configuration can never mutate the checkout.
	file(GLOB po_files CONFIGURE_DEPENDS ${arg_PO_DIR}/*.po)
	set(metainfo_po_dir "${CMAKE_CURRENT_BINARY_DIR}/${PROJECT_NAME}-metainfo-po")
	file(MAKE_DIRECTORY "${metainfo_po_dir}")
	foreach(po_file ${po_files})
		get_filename_component(lang ${po_file} NAME_WE)
		list(APPEND linguas ${lang})
		configure_file(${po_file} "${metainfo_po_dir}/${lang}.po" COPYONLY)
	endforeach()
	list(JOIN linguas "\n" linguas_contents)
	file(WRITE "${metainfo_po_dir}/LINGUAS" "${linguas_contents}\n")
	...
	add_custom_command(
		...
		COMMAND ${GETTEXT_MSGFMT_EXECUTABLE}
			...
			-d ${metainfo_po_dir}
			-o ${desktop_file}
		DEPENDS ${arg_PO_DIR}/../${desktop_file}.in ${po_files} ${metainfo_po_dir}/LINGUAS
	)
	install(
	...
	add_custom_command(
		...
		COMMAND ${GETTEXT_MSGFMT_EXECUTABLE}
			...
			-d ${metainfo_po_dir}
			-o ${appdata_file}
		DEPENDS ${arg_PO_DIR}/../${appdata_file}.in ${po_files} ${metainfo_po_dir}/LINGUAS
	)
	install(
	...
	foreach(mimetype_file ${arg_MIMETYPES})
		add_custom_command(
			...
			COMMAND ${GETTEXT_MSGFMT_EXECUTABLE}
				...
				-d ${metainfo_po_dir}
				-o ${mimetype_file}
			DEPENDS ${arg_PO_DIR}/../${mimetype_file}.in ${po_files} ${metainfo_po_dir}/LINGUAS
		)
		install(
