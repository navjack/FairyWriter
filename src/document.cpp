	...
	, m_daily_progress(daily_progress)
{
	...
	// The engine owns the sole document model; QTextEdit is presentation only.
	m_text->setDocument(m_engine.document());
...
Document::~Document()
{
	// QTextEdit is only a view; detach it before the engine-owned document is
	// destroyed as Document's members are torn down.
	m_text->setDocument(nullptr);
...
void Document::loadTheme(const Theme& theme, int virtual_pixel_pitch)
{
	...
	p.setColor(QPalette::HighlightedText, (qGray(m_text_color.rgb()) > 127) ? theme.blackReplacement() : theme.foregroundColor());
	m_text->setPalette(p);
	...
	const int tab_width = theme.tabWidth() * pitch;
	...
	m_block_format.setTopMargin(theme.spacingAboveParagraph() * pitch);
	m_block_format.setBottomMargin(theme.spacingBelowParagraph() * pitch);
	...
	// Update text
	QFont font = SnesStyle::scaledDocumentFont(theme.textFont(), theme.documentFontPixels(), pitch);
...
bool Document::eventFilter(QObject* watched, QEvent* event)
{
	if (event->type() == QEvent::MouseMove) {
		...
		const bool command_modifier = key_event->modifiers().testFlag(Qt::ControlModifier)
			|| key_event->modifiers().testFlag(Qt::MetaModifier)
			|| key_event->modifiers().testFlag(Qt::AltModifier);
		if ((!command_modifier && !key_event->text().isEmpty())
...
{
	m_engine.syncCursor(m_text->textCursor());
