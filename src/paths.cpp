...
bool copyTree(const QDir& source, const QDir& destination)
{
	if (!destination.exists() && !destination.mkpath(".")) return false;
	for (const QFileInfo& entry : source.entryInfoList(QDir::NoDotAndDotDot | QDir::AllEntries)) {
		...
		if (entry.isDir()) {
			if (!copyTree(QDir(entry.absoluteFilePath()), QDir(target))) return false;
...
QString Paths::legacyFocusWriterPath()
{
	return oldDataPath();
}

bool Paths::copyLegacyProfile(const QString& source, const QString& destination)
{
	if (source.isEmpty() || destination.isEmpty() || QDir::cleanPath(source) == QDir::cleanPath(destination)) {
		return false;
	}
	return copyTree(QDir(source), QDir(destination));
}
