...
- Smoother motion: the emulated machine is driven at its true 60.0988 Hz rather
  ...
  second.
- The showcase video is generated by the application instead of screen-captured,
  and covers what 0.2.0 and 0.3.0 added: styles combining, paragraph alignment,
  and the SPC700 sound shaper. It has sound now, which a video about a sound
  editor rather needed. A scripted child mode drives the same player a real
  session runs and writes the cartridge's own framebuffer and the S-DSP's own
  output to raw streams, one emulated frame at a time — so one video frame is
  one DSP block, the two cannot drift, and the same commit reproduces the same
  file. Refreshing the showcase for a release is now a re-run rather than a
  re-shoot. See `docs/SHOWCASE.md`.
