...
private Q_SLOTS:
	...
	void updateSave();
	void applyChromeStep(int step);
	void toggleFocusPresentation(bool enabled);
	void toggleInterfacePinned(bool pinned);
	void updatePresentationDensity(int pitch);
	void setUiMagnification(int magnification);
...
private:
	...
	void initMenus();
	void stopDocumentCache();
	void updateChromeOverlays();
...
private:
	...
	QTimer* m_save_timer;
	FocusPresentation* m_focus_presentation;
	ChromeDitherOverlay* m_top_chrome_fade;
	ChromeDitherOverlay* m_bottom_chrome_fade;
