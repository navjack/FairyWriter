...
TextCodec* codecForCodePage(qint32 value)
{
	...
	// An RTF codepage is untrusted file metadata. Unsupported legacy encodings
	// must not leave m_codec null (the tokenizer always dereferences it), and
	// ASCII should remain readable even when the original non-ASCII script is
	// outside the resident cartridge glyph set. Latin-1 is the deterministic
	// single-byte fallback on every platform.
	return codec ? codec : TextCodec::codecForName("ISO-8859-1");
}
