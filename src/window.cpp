...
// Paint presentation-background pixels over the chrome in a 4x4 ordered
// pattern. Intermediate frames never use alpha blending or off-palette colors.
class ChromeDitherOverlay final : public QWidget
{
public:
	explicit ChromeDitherOverlay(Stack* stack, QWidget* parent)
		: QWidget(parent)
		, m_stack(stack)
		, m_step(FocusPresentation::VisibleStep)
	{
...
protected:
	...
	{
		const QPixmap& background = m_stack->presentationBackground();
		...
		const int pitch = std::max(1, m_stack->presentationScale());
		const QPoint source_origin = m_stack->mapFrom(parentWidget(), geometry().topLeft());
		for (int y = 0; y < height(); y += pitch) {
			for (int x = 0; x < width(); x += pitch) {
				const int virtual_x = (source_origin.x() + x) / pitch;
				const int virtual_y = (source_origin.y() + y) / pitch;
				if (!FocusPresentation::ditherCovered(virtual_x, virtual_y, m_step)) {
					continue;
				}
				const QRect target(x, y, std::min(pitch, width() - x), std::min(pitch, height() - y));
				const QRect source(source_origin + target.topLeft(), target.size());
...
Window::Window(const QStringList& command_line_files)
	...
	, m_footer(nullptr)
	, m_focus_presentation(nullptr)
	, m_top_chrome_fade(nullptr)
	, m_bottom_chrome_fade(nullptr)
	, m_fullscreen(true)
	, m_save_positions(true)
{
	...
	// Keep the menu in FairyWriter's full-width pixel chrome on every platform.
	...
	connect(m_documents, &Stack::presentationScaleChanged, this, &Window::updatePresentationDensity);
	m_focus_presentation = new FocusPresentation(this);
	m_top_chrome_fade = new ChromeDitherOverlay(m_documents, this);
	m_bottom_chrome_fade = new ChromeDitherOverlay(m_documents, this);
	connect(m_focus_presentation, &FocusPresentation::chromeStepChanged, this, &Window::applyChromeStep);
	connect(m_documents, &Stack::editorTyped, m_focus_presentation, &FocusPresentation::editorTyped);
	connect(m_documents, &Stack::pointerMoved, m_focus_presentation, &FocusPresentation::pointerMoved);
	connect(m_documents, &Stack::sidePanelVisible, m_focus_presentation, &FocusPresentation::setInteractionActive);
	QSettings presentation_settings;
	m_focus_presentation->setAutoHideEnabled(presentation_settings.value("Presentation/AutoHide", true).toBool());
	m_focus_presentation->setPinned(presentation_settings.value("Presentation/InterfacePinned", false).toBool());
	...
	updateMargin();
	applyChromeStep(FocusPresentation::VisibleStep);
	m_tabs->removeTab(0);
	...
	if (!m_document_cache->isWritable()) {
		...
		if (!filenames.isEmpty()) {
			...
			mbox.setText(tr("FairyWriter was not shut down cleanly."));
...
{
	stopDocumentCache();
	// Documents unregister themselves from the process-wide DocumentWatcher.
	// Destroy their owning stack explicitly while the watcher child still
	// exists; QObject's creation-order child teardown would destroy the watcher
	// first and leave every remaining document with a dangling singleton.
	delete m_documents;
	m_documents = nullptr;
}
...
void Window::stopDocumentCache()
{
	if (m_document_cache_thread && m_document_cache_thread->isRunning()) {
...
void Window::addDocuments(const QStringList& files, const QStringList& datafiles, const QStringList& positions, int active, bool show_load)
{
	...
	// Hide interface
	m_focus_presentation->hideImmediately();
...
bool Window::event(QEvent* event)
{
	if (event->type() == QEvent::WindowBlocked) {
		m_focus_presentation->setInteractionActive(true);
	} else if (event->type() == QEvent::WindowUnblocked) {
		m_focus_presentation->setInteractionActive(false);
	}
...
bool Window::eventFilter(QObject* watched, QEvent* event)
{
	if (event->type() == QEvent::KeyPress || event->type() == QEvent::KeyRelease) {
		...
		if (key->key() == Qt::Key_Alt || key->key() == Qt::Key_F10) {
...
void Window::setLanguageClicked()
{
	...
	m_documents->showInSidePanel(dialog, tr("Dictionaries"));
}
...
void Window::toggleFocusPresentation(bool enabled)
{
	QSettings().setValue("Presentation/AutoHide", enabled);
	m_focus_presentation->setAutoHideEnabled(enabled);
}

//-----------------------------------------------------------------------------

void Window::toggleInterfacePinned(bool pinned)
{
	QSettings().setValue("Presentation/InterfacePinned", pinned);
	m_focus_presentation->setPinned(pinned);
}

//-----------------------------------------------------------------------------

void Window::applyChromeStep(int step)
{
	updateChromeOverlays();
	if (step == FocusPresentation::HiddenStep) {
		m_documents->setHeaderVisible(false);
		m_documents->setFooterVisible(false);
		m_top_chrome_fade->hide();
		m_bottom_chrome_fade->hide();
		return;
	}

	m_documents->setHeaderVisible(true);
	m_documents->setFooterVisible(true);
	if (step == FocusPresentation::VisibleStep) {
		m_top_chrome_fade->hide();
		m_bottom_chrome_fade->hide();
		return;
	}

	m_top_chrome_fade->setChromeStep(step);
	m_bottom_chrome_fade->setChromeStep(step);
	m_top_chrome_fade->show();
	m_bottom_chrome_fade->show();
	m_top_chrome_fade->raise();
	m_bottom_chrome_fade->raise();
}

//-----------------------------------------------------------------------------

void Window::updateChromeOverlays()
{
	if (!m_footer || !m_top_chrome_fade || !m_bottom_chrome_fade) {
		return;
	}
	const int header_height = centralWidget()->mapToParent(QPoint(0, 0)).y();
	const QPoint footer_position = m_footer->mapTo(this, QPoint(0, 0));
	m_top_chrome_fade->setGeometry(0, 0, width(), std::max(0, header_height));
	m_bottom_chrome_fade->setGeometry(0, footer_position.y(), width(), m_footer->height());
}
...
{
	ThemeManager* manager = new ThemeManager(*m_sessions->current()->data(), this);
	connect(manager, &ThemeManager::themeSelected, m_documents, &Stack::themeSelected);
	connect(manager, &ThemeManager::editRequested, this, [this, manager](const Theme& theme, bool is_new) {
		auto* editor = new ThemeDialog(theme, is_new, this);
		connect(editor, &ThemeDialog::previewChanged, this, [this](const Theme& draft) {
			if (m_documents->themePreviewActive()) {
				m_documents->updateThemePreview(draft);
			} else {
				m_documents->beginThemePreview(draft);
			}
		});
		connect(editor, &ThemeDialog::previewCanceled, m_documents, &Stack::cancelThemePreview);
		connect(editor, &ThemeDialog::themeSaved, this, [this](const Theme& saved) {
			m_sessions->current()->setTheme(saved.id(), false);
			QSettings* settings = m_sessions->current()->data();
			settings->setValue("ThemeManager/Theme", saved.id());
			settings->setValue("ThemeManager/ThemeDefault", false);
			m_documents->commitThemePreview(saved);
		});
		connect(editor, &QDialog::finished, editor, &QObject::deleteLater);
		connect(editor, &QDialog::finished, manager, &QObject::deleteLater);
		m_documents->beginThemePreview(editor->draft());
		m_documents->showInSidePanel(editor, tr("Theme Editor"));
	});
	connect(manager, &QDialog::finished, manager, &QObject::deleteLater);
	m_documents->showInSidePanel(manager, tr("Themes"));
}
...
{
	PreferencesDialog* dialog = new PreferencesDialog(m_daily_progress, this);
	connect(dialog, &QDialog::accepted, this, &Window::loadPreferences);
	connect(dialog, &QDialog::finished, dialog, &QObject::deleteLater);
	m_documents->showInSidePanel(dialog, tr("Preferences"));
}
...
{
	QMessageBox::about(this, tr("About FairyWriter"), QString(
		"<p align='center'><big><b>%1 %2</b></big><br/>%3<br/><small>%4<br/>%5<br/>%6</small></p>"
		"<p align='center'>%7<br/><small>%8</small></p>")
		.arg(tr("FairyWriter"), QApplication::applicationVersion(),
			tr("A happy, SNES-inspired focus word processor"),
			tr("Derived from FocusWriter by Graeme Gott"),
...
void Window::setLocaleClicked()
{
	...
	connect(dialog, &QDialog::finished, dialog, &QObject::deleteLater);
	m_documents->showInSidePanel(dialog, tr("Application Language"));
}
...
void Window::loadPreferences()
{
	...
	m_documents->setWallpaperMotionEnabled(Preferences::instance().wallpaperMotion());
	...
	applyChromeStep(m_focus_presentation->chromeStep());
	for (int i = 0, count = m_documents->count(); i < count; ++i) {
...
void Window::updatePresentationDensity(int pitch)
{
	SnesStyle::setVirtualPixelPitch(pitch);
	const int ui_pitch = SnesStyle::uiPixelPitch();
	const QSize icon_size(8 * ui_pitch, 8 * ui_pitch);
...
void Window::setUiMagnification(int magnification)
{
	SnesStyle::setUiMagnification(magnification);
	QSettings().setValue("Presentation/UiMagnification", SnesStyle::uiMagnification());
	updatePresentationDensity(m_documents->presentationScale());
	QTimer::singleShot(0, this, &Window::updateMargin);
}
...
void Window::initMenus()
{
	...
	connect(m_actions["ManageSessions"], &QAction::triggered, this, [this] {
		m_documents->showInSidePanel(m_sessions, tr("Sessions"));
	});
	...
	m_actions["FocusPresentation"] = view_menu->addAction(tr("&Focus Presentation"), this, &Window::toggleFocusPresentation);
	m_actions["FocusPresentation"]->setCheckable(true);
	m_actions["FocusPresentation"]->setChecked(m_focus_presentation->autoHideEnabled());
	m_actions["FocusPresentation"]->setStatusTip(tr("Automatically hide both interface bars after typing"));
	ActionManager::instance()->addAction("FocusPresentation", m_actions["FocusPresentation"]);
	m_actions["PinInterface"] = view_menu->addAction(tr("&Pin Interface"), this, &Window::toggleInterfacePinned);
	m_actions["PinInterface"]->setCheckable(true);
	m_actions["PinInterface"]->setChecked(m_focus_presentation->pinned());
	m_actions["PinInterface"]->setStatusTip(tr("Keep the menu and command bars visible"));
	ActionManager::instance()->addAction("PinInterface", m_actions["PinInterface"]);
	QMenu* panel_side_menu = view_menu->addMenu(tr("Panel &Side"));
	QAction* panel_right = panel_side_menu->addAction(tr("&Right"));
	QAction* panel_left = panel_side_menu->addAction(tr("&Left"));
	panel_right->setCheckable(true);
	panel_left->setCheckable(true);
	QActionGroup* panel_side_group = new QActionGroup(this);
	panel_side_group->addAction(panel_right);
	panel_side_group->addAction(panel_left);
	panel_side_group->setExclusive(true);
	panel_right->setChecked(m_documents->panelSide() == PanelSide::Right);
	panel_left->setChecked(m_documents->panelSide() == PanelSide::Left);
	connect(panel_right, &QAction::triggered, this, [this] { m_documents->setPanelSide(PanelSide::Right); });
	connect(panel_left, &QAction::triggered, this, [this] { m_documents->setPanelSide(PanelSide::Left); });
	QMenu* ui_scale_menu = view_menu->addMenu(tr("Interface &Scale"));
	QActionGroup* ui_scale_group = new QActionGroup(this);
	ui_scale_group->setExclusive(true);
	for (const int factor : {1, 2, 4}) {
		QAction* scale = ui_scale_menu->addAction(tr("%1x").arg(factor));
		scale->setCheckable(true);
		scale->setData(factor);
		scale->setChecked(SnesStyle::uiMagnification() == factor);
		ui_scale_group->addAction(scale);
	}
	connect(ui_scale_group, &QActionGroup::triggered, this, [this](QAction* action) {
		setUiMagnification(action->data().toInt());
	});
	...
	tools_menu->addSeparator();
	m_actions["Timers"] = tools_menu->addAction(QIcon::fromTheme("appointment", QIcon::fromTheme("chronometer")), tr("&Timers..."), this, [this] {
		m_documents->showInSidePanel(m_timers, tr("Timers"));
	});
	m_actions["Symbols"] = tools_menu->addAction(QIcon::fromTheme("character-set"), tr("S&ymbols..."), m_documents, &Stack::showSymbols);
	m_actions["DailyProgress"] = tools_menu->addAction(QIcon::fromTheme("view-calendar"), tr("&Daily Progress"), this, [this] {
		m_documents->showInSidePanel(m_daily_progress_dialog, tr("Daily Progress"));
	});
	...
	m_actions["ShowToolbar"]->setChecked(QSettings().value("Toolbar/Shown", true).toBool());
	m_actions["ShowToolbar"]->setStatusTip(tr("Show or hide the toolbar"));
#ifdef Q_OS_MAC
	m_actions["ShowToolbar"]->setShortcut(QKeySequence(Qt::META | Qt::SHIFT | Qt::Key_T));
#else
	m_actions["ShowToolbar"]->setShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_T));
#endif
	m_actions["ShowToolbar"]->setShortcutContext(Qt::WindowShortcut);
	settings_menu->addAction(m_actions["ShowToolbar"]);
	connect(m_actions["ShowToolbar"], &QAction::toggled, m_toolbar, &QToolBar::setVisible);
	connect(m_toolbar, &QToolBar::visibilityChanged, this, [this](bool visible) {
		const QSignalBlocker blocker(m_actions["ShowToolbar"]);
		m_actions["ShowToolbar"]->setChecked(visible);
		QSettings().setValue("Toolbar/Shown", visible);
		updateMargin();
	});
#ifndef Q_OS_MAC
	QAction* action = settings_menu->addAction(tr("Show &Menu Icons"), this, &Window::toggleMenuIcons);
	...
	// Menus use the same focus-presentation interaction pin on macOS and Linux.
	const QList<QMenu*> top_menus{ file_menu, edit_menu, format_menu, view_menu, tools_menu, settings_menu, help_menu };
	for (QMenu* menu : top_menus) {
		...
		connect(menu, &QMenu::aboutToHide, this, [this] {
			QTimer::singleShot(0, this, [this] {
				if (!qApp->activePopupWidget()) {
					m_focus_presentation->setInteractionActive(false);
					if (m_documents->currentDocument()) {
						m_documents->currentDocument()->text()->setFocus();
					}
				}
			});
